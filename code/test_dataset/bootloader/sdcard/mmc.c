...
static int mmc_send_status(struct mmc *mmc, int timeout)
{
	...
	if (timeout <= 0) {
		xprintf("Timeout waiting card ready\n\r");
		return TIMEOUT;
	}
...
retry_scr:
	...
	xprintf("before src, mmc_card_caps:%x\n\r", mmc->card_caps);
	xprintf("scr:%x\n\r", mmc->scr[0]);
	if (mmc->scr[0] & SD_DATA_4BIT)
		mmc->card_caps |= MMC_MODE_4BIT;
	xprintf("after src, mmc_card_caps:%x\n\r", mmc->card_caps);
	...
	while (timeout--) {
		...
		// xprintf("switch status 7 %08x\n\r", switch_status[7]);
		// xprintf("switch status 3 %08x\n\r", switch_status[3]);
		// xprintf("switch status 4 %08x\n\r", switch_status[4]);
...
static int mmc_startup(struct mmc *mmc)
{
	...
	char test_csd[512];
	int timeout = 1000;
	xprintf("mmc_startup, mmc_bus_width:%d\n\r", mmc->bus_width);
	xprintf("mmc_startup, mmc_card_caps:%x\n\r", mmc->card_caps);
	...
	xprintf("before sd_change_freq, mmc_card_caps:%x\n\r", mmc->card_caps);
	if (IS_SD(mmc))
		err = sd_change_freq(mmc);
	else
		err = mmc_change_freq(mmc);
	xprintf("after sd_change_freq, mmc_card_caps:%x\n\r", mmc->card_caps);
	if (err) {
		xprintf("%d", __LINE__);
		return err;
	}
...
static int mmc_read_blocks(struct mmc *mmc, void *dst, size_t start, size_t blkcnt)
{
	...
	if (mmc_send_cmd(mmc, &cmd, &data)) {
		xprintf("mmc_read_blocks->mmc_send_cmd\n\r");
		return 0;
	}
	...
	if (blkcnt > 1) {
		...
		if (mmc_send_cmd(mmc, &cmd, NULL)) {
			xprintf("mmc fail to send stop cmd\n");
			return 0;
		}
	}
...
int mmc_init(struct mmc *mmc)
{
	...
	if (err == TIMEOUT) {
		...
		if (err) {
			xprintf("Card did not respond to voltage select!\n\r");
			return UNUSABLE_ERR;
		}
	}
...
size_t mmc_bread(struct mmc *mmc, size_t start, size_t blkcnt, void *dst)
{
	...
	if ((start + blkcnt) > mmc->capacity / mmc->read_bl_len) {
		xprintf("start + blkcnt\n\r");
		// xprintf("MMC: block number 0x%lx exceeds max(0x%lx)\n",
			// start + blkcnt, mmc->capacity / mmc->read_bl_len);
		return 0;
	}

	if (mmc_set_blocklen(mmc, mmc->read_bl_len)) {
		xprintf("mmc_set_blocklen\n\r");
		return 0;
	}
...
{
	// xprintf("Device: %s\n\r", mmc->name);
	// xprintf("Manufacturer ID: %x\n\r", mmc->cid[0] >> 24);
	// xprintf("OEM: %x\n\r", (mmc->cid[0] >> 8) & 0xffff);
	// xprintf("Name: %c%c%c%c%c \n\r", mmc->cid[0] & 0xff,
	// 		(mmc->cid[1] >> 24), (mmc->cid[1] >> 16) & 0xff,
	// 		(mmc->cid[1] >> 8) & 0xff, mmc->cid[1] & 0xff);

	// xprintf("Tran Speed: %d\n\r", mmc->tran_speed);
	// xprintf("Rd Block Len: %d\n\r", mmc->read_bl_len);

	// xprintf("%s version %d.%d\n\r", IS_SD(mmc) ? "SD" : "MMC",
	// 		(mmc->version >> 4) & 0xf, mmc->version & 0xf);

	// xprintf("High Capacity: %s\n\r", mmc->high_capacity ? "Yes" : "No");
	// xprintf("Capacity: %ld\n\r", mmc->capacity);

	// xprintf("Bus Width: %d-bit\n\r", mmc->bus_width);
}
